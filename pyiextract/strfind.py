...
def best_fit_index(text: str, substring: str) -> int:
    if len(substring) > len(text):
        return 0
    return sorted([{"index": i, "dist": distance(substring, text[i:len(substring)])} for i in range(0, len(text) - len(substring))], key=lambda x: x["dist"])[0]["index"]
...
def token_ids_span(span: typing.Any, entity_idx: int, entity_text: str) -> typing.Tuple[int, int]:
    idx_start = 0
    idx_end = 0
    for token in span:
...
def extract_entities_span(span: typing.Any, head_entity_text: str, relation_text: str, tail_entity_text: str) -> typing.Tuple[typing.Any, typing.Any]:
    ...
    head_entity_idx_start, head_entity_idx_end = token_ids_span(span, head_entity_idx, head_entity_text)
    tail_entity_idx_start, tail_entity_idx_end = token_ids_span(span, tail_entity_idx, tail_entity_text)
